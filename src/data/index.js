...
export function priceLine(l) {
	if (l.price === 0) {
		if (l.propertyId) return 'Lodge';
		if (l.productId) return 'Product';
		if (l.serviceId) return 'Service';
		return TYPE_LABEL[l.type] || 'Free';
	}
	
	let suffix = '';
	if (l.propertyId) {
		suffix = '/yr';
	} else if (l.serviceId && l.perUnit) {
		suffix = ` / ${l.perUnit}`;
	} else if (l.unit) {
		suffix = l.unit === 'year' ? '/yr' : ` / ${l.unit}`;
	}
	
	return fmtMoney(l.price) + suffix;
}
