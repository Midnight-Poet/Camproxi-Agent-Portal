...
export default function NotificationToast({ toast, onClose }) {
	...
	useEffect(() => {
		if (toast) {
			...
			return () => {
				console.log('[NotificationToast] Cleaning up timer');
				clearTimeout(timer);
			};
		}
	}, [toast, onClose]);
	...
	return (
		<div className="fixed top-6 left-[5%] right-[5%] sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-[9999] pointer-events-none flex justify-center">
			<div 
				...
				className={`pointer-events-auto bg-white/95 backdrop-blur-xl border-[1.5px] border-white shadow-lg2 rounded-[20px] p-4 flex items-start gap-3.5 w-full sm:w-[400px] animate-pop ${toast.link ? 'cursor-pointer hover:bg-white' : ''}`}
			>
