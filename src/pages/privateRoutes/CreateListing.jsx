...
import { inputCls } from '../../components/shared/Field';
import Loading from '../../components/Loading';
import Layout from '../../components/Layout';
import SectionLabel from '../../components/listings/SectionLabel';
...
import { useCreatePropertyMutation } from '../../redux/api/propertyApiSlice';
...
export default function CreateListing({ setState, id }) {
	...
	const handleSave = async () => {
		try {
            ...
            if (userInfo?.category === 'business') {
                formData.append('name', title);
                formData.append('businessCategory', category);
                formData.append('description', desc);
                formData.append('price', price);
                formData.append('delivery', JSON.stringify({ option: deliveryOption }));
                
                await createProduct(formData).unwrap();
            } else if (userInfo?.category === 'agent') {
                formData.append('name', title);
                formData.append('address', area);
                formData.append('roomType', room);
                formData.append('description', desc);
                formData.append('price', price);
                formData.append('unitQuantity', 1);
                formData.append('location', JSON.stringify({ lat: 0, lng: 0 }));
                amenities.forEach(a => formData.append('amenities', a));
                
                await createProperty(formData).unwrap();
            } else if (userInfo?.category === 'services') {
                formData.append('name', title);
                formData.append('address', area);
                formData.append('serviceCategory', cat);
                formData.append('description', desc);
                formData.append('price', price);
                formData.append('perUnit', PRICE_UNIT[type] || 'Hour');
                formData.append('time', JSON.stringify({ startTime: '08:00', endTime: '18:00' }));
						...
						<div className='flex flex-wrap justify-between items-center gap-5'>
							...
							<div className='w-[48%]'>
								...
								<div className='relative'>
									<span className='absolute left-3.5 top-1/2 -translate-y-1/2 text-muted z-10 font-bold'>
										₦
									</span>
									<input
